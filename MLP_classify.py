...
def main():
    ...
    print(f"  Input: {X_tr.shape[1]} clinical features")
    print(f"{'='*60}")

    history  = {'train_loss': [], 'val_loss': [], 'val_acc': [], 'val_f1': []}
    best_f1  = 0.0
    patience = 0
    ckpt     = os.path.join(CKPT_DIR, 'mlp_best.pt')
    ...
    fig.tight_layout()
    p = os.path.join(RESULTS_DIR, 'mlp_confusion_matrix.png')
    fig.savefig(p, dpi=150, bbox_inches='tight'); plt.close(fig)
    print(f"  Saved: {p}")

    fig, ax = plt.subplots(figsize=(7, 6))
    ax.plot(fpr, tpr, 'b-', lw=2, label=f'MLP (AUC={auc_score:.3f})')
    ax.plot([0,1],[0,1],'k--',alpha=0.5)
    ax.set_xlabel('False Positive Rate'); ax.set_ylabel('True Positive Rate')
    ax.set_title('ROC Curve — MLP'); ax.legend(loc='lower right')
    p = os.path.join(RESULTS_DIR, 'mlp_roc_curve.png')
    fig.savefig(p, dpi=150, bbox_inches='tight'); plt.close(fig)
    print(f"  Saved: {p}")

    fig, axes = plt.subplots(1, 3, figsize=(15, 4))
    fig.suptitle('MLP Training History', fontsize=13)
    ...
    fig.tight_layout()
    p = os.path.join(RESULTS_DIR, 'mlp_training_curves.png')
    fig.savefig(p, dpi=150, bbox_inches='tight'); plt.close(fig)
    print(f"  Saved: {p}")
